...
Dockerfile存在许多指令，此处逐个进行详细解析和用法
...
正确的使用姿势是，在镜像构建时，使用的版本号，如`ARG VERSION=5.5.0`或者需要的端口号`ARG PORT=8088`等只需要在build临时使用的变量在表示的时候使用`ARG`即可，另一方面`ENV`在使用的时候是可以被带到镜像里面的变量，如设置Java的环境变量，此时则需要使用`ENV`来进行设置。总结一下就是：

`ARG` 是在 build 的时候存在的, 可以在 Dockerfile 中当做变量来使用，并且可以传参数进行替换。
